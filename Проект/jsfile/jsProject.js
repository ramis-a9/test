...
// !!!!!!!!!!!!! УСТАНОВКА ТАЙМЕРА !!!!!!!!!!!!!!!!!!!!!!!!
    ...
    dotsWrap.addEventListener('click', function(event) {
        for (let i = 0; i < dots.length + 1; i++) {
            if (event.target.classList.contains('dot') && event.target == dots[i-1]) { // При делегировании событий мы проверяем элемент на опр параметры И только потом что то делаем. Тут мы проверяем, что мы кликнули именно на точку
                currentSlide(i); 
            }
        }
    });
