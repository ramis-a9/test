...
// !!!!!!!!!!!!! УСТАНОВКА ТАЙМЕРА !!!!!!!!!!!!!!!!!!!!!!!!
    ...
    // в любой форме для отправки должна быть или Button или Input для отправки инфомрации
     ...
     form.addEventListener('submit', function (event) { // submit - поддтверждение нашей формы - это событие происходит только тогда, когда наша форма отправляется
    // большая ошибка, если мы будем вешать это событие на кнопку, отметим, мы сейчас вешаем событие на всю форму! Нам нужно отслеживать, когда форма отправляется на сервер
        // так как у нас тут стандартное поведение браузера, если нажать на кнопку отправки заявки, страница начнет отправлять запрос и перезагрузится полностью, нам это не нужно
        event.preventDefault(); // тут мы отменили стандартное поведение браузера, и теперь страница не отправляет форму и не перезагружается, стандартное поведение отключено 
        // далее нам нужно оповсестить пользователя о статусе
        form.appendChild(statusMessage); // тут мы добавили новый div на страницу 
        
        // теперь мы должны отправить данные на сервер
        let request = new XMLHttpRequest(); //Создали новый контструктор с объектом целевым
        ...
        request.setRequstHeader ('Content-Type', 'application/x-www-form-urlecoded'); //Далее тут идет настройка заголовков Http. Мы копировали из буфера обмена. - тут написано, наш контент, котоырй мы будем отправлять на сервер, будет содержать данные, которые мы взяли из формы.

        // далее нужно получить данные, которые ввел пользователь

        // Встроеенный обхект, работает во всех браузерах. Нам не всегда нужно передавать данные в jsom формате. FormData создаст структуру, которую ввел польщователь в формета ключа-значение.
        // Чтобы правильно отправлять данные куда то на сервер, нужно чтобы в input-ах стояли параметры name . Именно из Атрибута name мы будем формировать ключ, а значение уже введет пользователь. У нас там в классе main-form все есть.
        
        // Получаем данные, Которые ввел пользователь через Формдата!
        let formData = new FormData(form); // тут мы задали новые данные, во внутрь объекта засунули ту самую форму, из которой мы будем получать данные !
        // POST - может отправлять данные, а GET- только получает. Для ПОСТ - нужен body, в данном случае это просто данные с формыю
        request.send(formData); // Здесь мы отправляем на сервер данные, которые ввел пользователь. Тут есть Боди - тк мы отправляем данные, поэтому пишем что именно отправляем
        // Если у меня нормальный сервер, а не лайтовый, я отпраляю данные, захожу во вкладку Nework в браузере (где консоль). Далее внизу ищу server.php я его как раз и указывал при создании requst. И внизу я увижу переданную инфомрацию
        
        // ТЕПЕРЬ нужно сказать пользователю, что все отправлено
        request.addEventListener('readystatechange', function() { //события прописывается для того, чтобы мы могли наблюдать за изменениями состояния нашего запроса
            if (request.readyState < 4) {  // Это ты помнишь, статусы ответа
                statusMessage.innerHTML = message.loading // Это мы задавали выше и вносили на страницу как ДИВ. innerHTML - вроде запись
        ...
        // Обычно после такого, все наши импуты очищаются и нам нужно здесь сделать тоже саоме

        for (let i = 0; i < input.length; i++) {
            input[i].value = ''; // тут мы очистили после нашего submit все наши inputa в этом окне.
        }; // Теперь инпут полностью очистился (то, где мы ввели те или иные данные)

        // Сейчас мы отправили данные в обычном формате, как это воспринимает php код.
        // Под JSON :
        // 1) Меняется заголовок - мы должны сказать , что отправятся json файлы 
            //  requst.setRequstHeader('Content-type', 'application/json; charset=utf-8');
        // 2) Данные с формы переводим в json формат
            // У нас есть объект formData, но мы не можем его закинуть просто так в json, нужно сделать промежуточный объекты
            // let obj = {};
                // formData.forEach(value, key) {
                //     obj[key] = value; // Этим мы заполнили наш объект, нужно запомнить данный алгоритм
                // };
        // 3) Теперь в новом obj лежат все данные, которые ответил нам пользователь, мы форматируем в json 
            // let json = JSON.stringify(obj);
            // requst.send(json); //И просто отправляем на сервер данные
