...
// Классы
// в ES6 мы перехрдим к Классам
// Каждый объект в ЖС имеет прототип, который явл другиим объектом. Все объекты в JS наследуют методы и свойства от своего прототипа, чтобы это использовать мы можем создавать классы и уже потом плотить потомков
...
// spead оператор - оператор разоворота. Она разворачивает массив и превращает его в набор каких то данных
let video = ['youtube', 'vimeo', 'rutube'],  // Это массив !!
    blogs = ['worldpress', 'livejournal', 'blogger'],
    internet = [...video, ...blogs, 'vk', 'facebook'];

console.log(internet); // в таком виде они не расскрылись полностью,
// но если добавить спред оператор, который пишется как ... (три точки). То наш массив сразу полностью раскрывается и становится полным
// Именно поэтому он называетсяч оператором разворота ! тк он их разворачивает
...
let numbers = [2, 5, 7];
log(...numbers); // Здесь мы захотели использоваться массив в функции, но без спред оператора, функция бы не запустилась, а с ним, она сразу верно подняла необходимые переменные под себя
